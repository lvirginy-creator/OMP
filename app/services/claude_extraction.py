...
- Répartition par taille : si un article/couleur est décliné en plusieurs tailles avec une quantité par taille (tableau de type "Size/PCS" avec une colonne par taille : XS, S, M, L, XL, ou des pointures), essaie de créer UNE LIGNE PAR TAILLE NON VIDE plutôt qu'une seule ligne agrégée :
  ...
  - `quantity` = la quantité de CETTE seule taille (pas le cumul de toutes les tailles).
  - `unit_price_net` = le prix unitaire de la ligne (identique pour toutes les tailles du même article/couleur).
  - `line_total_net` = la quantité de cette taille × unit_price_net (pas le sous-total de toutes les tailles réunies).
  N'ajoute aucune ligne pour une case vide ou à 0 du tableau de tailles.
  Exemple : une ligne de facture pour la référence "JYTZ8659(BX2159+CK2186)", couleur "Black", libellé "Removable Pads Y Back Yoga Tank Top", avec le tableau XS=2 S=2 M=2 L=2 XL=(vide) et un prix unitaire de 19,25 $, donne 4 lignes (aucune pour XL) :
